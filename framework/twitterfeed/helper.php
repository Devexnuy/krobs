...
class CthTwitterHelper {
   ...
   // Your Twitter App Consumer Key
	...
	private function consoleDebug($message) {
		if($this->debug === true) {
			$this->message .= 'tweet.js1: '.$message."\n";
	...
	/*
		...
		*/			
		public function prepareTweet($string) {
			...
			$replacement = '<a class="tweet_hashtag" href="$1">$1</a>';
			$string = preg_replace($pattern, $replacement, $string);

			//Search
				$pattern = '/[\#]+([A-Za-z0-9-_]+)/i';
				$replacement = ' <a  class="tweet_search" href="http://search.twitter.com/search?q=$1">#$1</a>';
				$string = preg_replace($pattern, $replacement, $string);
			
			// Mention
			$pattern = '/\s[\@]+([A-Za-z0-9-_]+)/i';
			$replacement = ' <a  class="tweet_mention" href="http://twitter.com/$1"><span class="at">@</span>$1</a>';
			$string = preg_replace($pattern, $replacement, $string);
/*
			//Mention
